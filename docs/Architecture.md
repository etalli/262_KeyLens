...
### Why KeyLens cannot be distributed via the Mac App Store

KeyLens uses `CGEventTap` (CoreGraphics) to intercept system-wide input events. This API operates at the kernel/CoreGraphics layer and captures every keypress and mouse event across the entire system, regardless of which application is focused. It is the only macOS API that supports cross-process, system-wide event monitoring from user space.

The Mac App Store enforces a mandatory sandbox on all submitted apps. The sandbox restricts each app to its own process space and explicitly prohibits observing input events generated by other processes. Apple provides no public entitlement that grants a sandboxed app cross-process `CGEventTap` access. Submissions that attempt to bypass this restriction are rejected during App Review.

This is a well-known constraint for the category: established keyboard monitoring utilities such as Karabiner-Elements are similarly absent from the App Store for the same reason.

### Required distribution path: Developer ID + Notarization

The correct distribution path for KeyLens is **Developer ID signing + Apple Notarization**, available to members of the Apple Developer Program ($99/year).

| Step | What it does |
|------|--------------|
| `codesign --deep --sign "Developer ID Application: …"` | Attaches a Developer ID certificate to the `.app` bundle |
| `xcrun notarytool submit KeyLens.dmg.zip --wait` | Uploads the archive to Apple for automated malware scanning |
| `xcrun stapler staple KeyLens.dmg` | Attaches the notarization ticket to the DMG container for offline Gatekeeper validation |
| `xcrun stapler staple KeyLens.app` | Optionally staples the ticket to the app bundle inside the DMG as well |

After notarization, macOS Gatekeeper recognises the app as being from a verified developer. Users can install it with a normal double-click — the `sudo xattr -rd com.apple.quarantine` workaround in the current installation instructions is no longer needed.

### What notarization does not change

- Users must still grant **Accessibility** permission (System Settings → Privacy & Security → Accessibility) before keystroke monitoring begins. This is a macOS requirement for any app using `CGEventTap` regardless of signing status.
- The `CGEventTap` architecture in `KeyboardMonitor.swift` remains unchanged.
- The `build.sh` pipeline would need `codesign`, `notarytool submit`, and `stapler staple` steps added after the bundle is created.
