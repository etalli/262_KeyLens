...
**Q2. アプリが Dock にアイコンを表示せず、メニューバー常駐アプリとして動作するための設定は、どのような仕組みで実現されていますか？（2つの要素を答えてください）**
...
**Q2. アプリが Dock にアイコンを表示せず、メニューバー常駐アプリとして動作するための設定は、どのような仕組みで実現されていますか？（2つの要素を答えてください）**
- **正解：** ① `Info.plist` の `LSUIElement = true`（プロセスをエージェントとしてマーク） ② `KeyLensApp.swift` の `MenuBarExtra { ... }.menuBarExtraStyle(.window)`（SwiftUI App プロトコルによるエントリポイント）
- **解説：** `main.swift` は存在せず、エントリポイントは `@main struct KeyLensApp: App` です。`LSUIElement` により Dock への表示とメニューバーの切り替えを防ぎ、`MenuBarExtra` によりメニューバーアイコンとポップアップパネルを定義しています。
...
- **解説：** SwiftUI の `MenuBarExtra (.window style)` を使用しています。メニューバーアイコンをクリックするとパネルが開き、その時点で SwiftUI View が描画されます。`MenuView` は描画のたびに `KeyCountStore.shared` から最新データを読み取るため、開いた瞬間に最新値が表示されます。`NSMenuDelegate.menuWillOpen` は現在使用されていません。
