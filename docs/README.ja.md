...
| **設定…** | メニュー表示のカスタマイズ・言語・通知・リセット・CSV 書き出し・バックアップ/リストア・ログフォルダを開く |
...
| セクション | 表示内容 |
|-----------|---------|
| **ライブ IKI** | 直近20打鍵のキー間隔（IKI）棒グラフ。0.5秒ごと更新。速度に応じて緑/橙/赤で色分け |
| **IKI 分布** | 打鍵間隔をバケット別にまとめたヒストグラム。自分のタイピングリズムの分布を確認できる |
| **日別合計** | 日ごとのキーストローク数の折れ線グラフ |
| **タイピング速度** | 日別平均 WPM の時系列グラフ |
| **バックスペース率** | 日別のバックスペース/修正率の時系列グラフ |
| **時間帯別分布** | 時刻（0〜23時）別の累積打鍵数棒グラフ |
| **月別合計** | カレンダー月ごとの打鍵数 |
